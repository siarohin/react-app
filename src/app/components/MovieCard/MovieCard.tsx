...
import NoPosterImage from "../../../../public/assets/noposter.png";

const MovieCard = (props: IMovieCardProps): ReactElement<IMovieCardProps> => {
  const { movie, editableAction, onClick } = props;

  const [isOpenedMovieMenu, setIsOpenedMovieMenu] = useState(false);

  const imgProps = useFallbackImage(movie?.posterPath, NoPosterImage);
...
function moviePropsAreEqual(prevMovie: IMovieCardProps, nextMovie: IMovieCardProps) {
  return prevMovie.movie?.id === nextMovie.movie?.id && prevMovie.movie?.releaseDate === nextMovie.movie?.releaseDate;
}

export default React.memo(MovieCard, moviePropsAreEqual);
